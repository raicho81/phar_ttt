...
class TTTTrainDataRedis(TTTTrainDataBase):
    ...
    def claim_pending_stream_messages(self, count):
        try:
            claimed = self.__r.xautoclaim(self.redis_states_updates_stream, self.redis_states_updates_stream_group, self.redis_stream_consumer_name, 3*60*1000, 0, count, True)
            if claimed != []:
